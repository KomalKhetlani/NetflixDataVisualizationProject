...
app.layout = html.Div(children=[
    html.Header(
        ...
        children=[
            ...
            html.Div("Netflix Data Visualisation", id='header-text')
        ]
    ...
    html.Div([
        dbc.Row([
            ...
            dbc.Col(
                ...
                dcc.Graph(id='pie-chart-ratings',figure=pieChart),
                html.Div("This pie chart indicates that majority of content on Netflix is of the rating TV-MA which means it is intended to be be viewed by mature, adult audiences and may be unsuitable for children under 17",id="graph1Insights")

                ], id='firstGraphDiv'), width=6
            ),
            dbc.Col(
                html.Div([

                    dcc.Graph(id='cc',figure=fig4),
                    html.Div('The line graph shows that there has been a decline in the content produced for both movies and TV shows on Netflix since 2018.',id='graph2Insights')
                ], id='secondGraphDiv'), width=6
        ...
        dbc.Row([
            dbc.Col(
                html.Div([
                    dcc.Graph(id='aa', figure=fig2),
                    html.Div("From the data, it is derived that the top 5 Directors on Netflix are Raul Campos, Jan Suter, Jay Karas, Marcus Raboy, Jay Chapman.", id='graph3Insights')

                ], id='thirdGraphDiv'),width=6

            ),

            dbc.Col(
                html.Div([
                dcc.Graph(id='bb',figure=fig3),
                html.Div("From the data, it is derived that the top 5 Actors on Netflix are Anupam Kher, Om Puri,Shah Rukh Khan, Takahiro Sakurai, Boman Irani.", id='graph4Insights')

                ], id='forthGraphDiv'),width=6
            )
        ]),
        ...
        dbc.Row([
            dbc.Col(
                html.Div([
                    dcc.Graph(id='dd',figure=fig5),
                    html.Div('The overall positive content is always greater than the neutral and negative content.', id='graph5Insights'),
                    html.Br()
                ])
